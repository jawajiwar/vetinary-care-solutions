...
pipeline { 
    ...
    stages {
     ...
     stage ('deploy s3') {
         steps {
             s3Upload consoleLogLevel: 'INFO', dontWaitForConcurrentBuildCompletion: false, entries: [[bucket: 'vetinary-care-solutions', excludedFile: '', flatten: false, gzipFiles: false, keepForever: false, managedArtifacts: false, noUploadOnFailure: false, selectedRegion: 'us-east-1', showDirectlyInBrowser: false, sourceFile: '**/*.war', storageClass: 'STANDARD', uploadFromSlave: false, useServerSideEncryption: false]], pluginFailureResultConstraint: 'SUCCESS', profileName: 'petclinic', userMetadata: []
         }
     }
...
stage ('QA') {
        steps {
       ansiblePlaybook becomeUser: null, credentialsId: '8735db1f-8125-48c0-ae78-254a8864f3ae', installation: 'ansible', inventory: 'hosts', playbook: 'deploy.yml', sudoUser: null
